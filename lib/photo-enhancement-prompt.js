...
PLATING REFERENCE: ${cat.plating}.`;
  ...
  // Core identity block — concise but firm (model already SEES the food in the init image)
  let identityBlock;
  if (hasIdentification) {
    identityBlock = `FOOD IDENTITY: ${parsed.dish ? parsed.dish : 'preserve original dish'}${parsed.cuisine ? ` (${parsed.cuisine})` : ''}. ${parsed.items ? `Contains: ${parsed.items}.` : ''} Do NOT change, swap, or replace any food item. Enhance quality only — same food, better photo.`;
  } else {
    identityBlock = `FOOD IDENTITY: Preserve the EXACT food items in this photo. Do NOT change or replace any food. Enhance quality only.`;
  }
...
- Garnishes look fresh-picked — bright herbs, precise microgreens, clean sauce dots.`;
  ...
  return `${tier} FOOD PHOTOGRAPHY ENHANCEMENT — Transform this into a ${fictionalLevel <= 30 ? 'polished restaurant' : fictionalLevel <= 70 ? 'professional food magazine' : 'Michelin-star editorial'} photo.
...
QUALITY: 4K, zero noise, professional DSLR shallow depth of field, print-ready.
