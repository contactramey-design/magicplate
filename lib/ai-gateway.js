...
Return as JSON:
{
  ...
  /**
   * Generate comprehensive structured data (JSON-LD) for SEO
   * Includes Menu, MenuItem, Restaurant, PriceSpecification
   */
  generateStructuredData(menuItem, restaurant) {
    ...
    if (menuItem.allergens && menuItem.allergens.length > 0) {
      baseSchema.allergen = menuItem.allergens;
    }
  ...
  /**
   * Generate comprehensive restaurant schema
   */
  generateRestaurantSchema(restaurant, menuItems = []) {
    return {
      "@context": "https://schema.org",
      "@type": "Restaurant",
      "name": restaurant.name,
      ...
      "servesCuisine": restaurant.cuisine || "Restaurant",
  ...
  /**
   * Analyze menu item from raw ingestion
   */
  async analyzeMenuItem(item, restaurant) {
    const prompt = `Analyze this raw menu item and extract structured data:

Raw Item: ${item.raw_text || item.name || 'Unknown'}
...
Extract and enrich:
1. Clean item name
2. Detailed description (if missing)
3. Price (verify/extract)
4. Category (Appetizer, Main, Dessert, etc.)
5. Ingredients (if mentioned)
6. Allergens (gluten, dairy, nuts, etc.)
7. Dietary tags (vegan, vegetarian, gluten-free)

Return as JSON with keys: name, description, price, category, ingredients (array), allergens (array), dietary_tags (array)`;

    try {
      const response = await this.generate('anthropic', 'claude-3-opus-20240229', [
        { role: 'system', content: 'You are an expert at analyzing restaurant menu items and extracting structured data.' },
        { role: 'user', content: prompt }
      ...
      try {
        ...
        return {
          ...item,
          ai_analyzed: false,
          note: 'AI analysis returned non-JSON response'
        };
      }
    } catch (error) {
...
Provide 5-7 specific, actionable SEO recommendations to improve local search visibility and rankings.`;
  ...
  /**
   * Generate on-page SEO elements
   */
  async generateOnPageSEO(restaurant, pageType, content) {
    const prompt = `Generate comprehensive on-page SEO elements for ${restaurant.name}:
...
Generate:
1. Meta title (50-60 characters, include location and cuisine)
2. Meta description (150-160 characters, compelling and keyword-rich)
3. H1 tag (main heading, include location)
4. H2 tags (3-5 subheadings)
5. Open Graph tags (title, description, image)
6. Twitter Card tags
7. Canonical URL
8. Focus keywords (3-5 primary keywords)

Return as JSON with keys: meta_title, meta_description, h1, h2 (array), og_title, og_description, og_image, twitter_card, canonical_url, focus_keywords (array)`;

    const response = await this.generate('anthropic', 'claude-3-sonnet-20240229', [
      { role: 'system', content: 'You are an expert on-page SEO specialist. Generate optimized meta tags and headings for local restaurant SEO.' },
      { role: 'user', content: prompt }
    ...
    try {
      ...
      return {
        meta_title: `${restaurant.name} - Best ${restaurant.cuisine || 'Restaurant'} in ${restaurant.address || ''}`,
        meta_description: `${restaurant.description || `Visit ${restaurant.name} for authentic ${restaurant.cuisine || 'cuisine'}.`}`,
        h1: `${restaurant.name}`,
        h2: ['Our Menu', 'About Us', 'Visit Us'],
        focus_keywords: [restaurant.name, restaurant.cuisine, restaurant.address]
      };
    }
  }
   ...
   */
  async generateContentSuggestion(restaurant, menuItem) {
    const prompt = `Create a blog-style content page for ${menuItem.name} at ${restaurant.name}:
...
Create:
1. Title (engaging, SEO-friendly)
2. URL slug (SEO-friendly)
3. Excerpt (1-2 sentences)
4. Content (300-500 words, story-driven, SEO-optimized)
5. Keywords (3-5 long-tail keywords)
6. Meta description

Format as engaging story about the dish, its origins, ingredients, and why it's special. Include location keywords naturally.`;

    const response = await this.generate('anthropic', 'claude-3-opus-20240229', [
      { role: 'system', content: 'You are an expert content writer creating engaging, SEO-optimized blog posts for restaurant menu items.' },
      { role: 'user', content: prompt }
    ...
    try {
      ...
      return {
        ...
        content: response,
        keywords: [menuItem.name, restaurant.name, restaurant.cuisine],
  ...
  /**
   * Optimize menu item for delivery platform
   */
  async optimizeForDeliveryPlatform(menuItem, restaurant, platform) {
    const prompt = `Optimize this menu item for ${platform} delivery platform:
...
${platform === 'doordash' ? '- Max 500 characters\n- Include location keywords\n- Highlight dietary options' : ''}
${platform === 'ubereats' ? '- Max 300 characters\n- Concise but descriptive\n- Include popular keywords' : ''}
${platform === 'grubhub' ? '- Max 400 characters\n- Focus on taste and quality\n- Include preparation details' : ''}

Optimize:
1. Platform-specific description (within character limit)
2. Platform name (if different)
3. SEO keywords for delivery search
4. Dietary tags
5. Preparation style mentions

Return as JSON with keys: platform_name, platform_description, seo_keywords (array), dietary_tags (array)`;

    const response = await this.generate('anthropic', 'claude-3-sonnet-20240229', [
      { role: 'system', content: 'You are an expert at optimizing restaurant menu items for delivery platforms while maintaining SEO value.' },
      { role: 'user', content: prompt }
    ...
    try {
      ...
      return {
        platform_name: menuItem.name,
        platform_description: menuItem.description || `${menuItem.name} - Fresh and delicious`,
        seo_keywords: [menuItem.name, restaurant.cuisine],
        dietary_tags: []
      };
    }
  }
